function options = eldaScoreBestWrapper(iid, options)
% wrapper for getting the windows that receive the best support from its neighborhoods
...
for d=dindex
    ...
    for i=1:length(classes)
        ...
        detfile = [tclasspath,'eldasbest-',options.canditmethod,'.mat'];
        lockpath = [tclasspath,'eldasbest-',options.canditmethod,'.lck/'];
        cachepath = [tclasspath,'eldasbest-',options.canditmethod,'.lock/'];
        
        if fileExists(prefile) && fileExists(prefile2) && ~fileExists(detfile)
            makeDirOrFail(cachepath);
            if iid == 1 && makeDirOrFail(lockpath)
                % do the detection
                clear eldas sumfeat imagelist bboxes
                load(pclasspath,'imagelist');
                load(prefile,'eldas','sumfeat');
                ...
                for j=1:num_img
                    ...
                    if ~fileExists(matpath) && makeDirOrFail(llockpath)
                        disp(matpath);
                        clear bscore bindex
                        [bscore, bindex] = scoreBest(eldas, j, sumfeat, bboxes{j}, imagelist{j}, options);

                        save(matpath,'bscore','bindex');
                        rmdir(llockpath);
                ...
                for j=1:num_img
                    ...
                    matpath = [cachepath,sprintf('%03d.mat',j)];
                    clear bscore bindex
                    load(matpath,'bscore','bindex');
                    BScore{j} = bscore;
                    bboxes{j} = Bboxes{j}(bindex,:);
                end

                save(detfile,'BScore','bboxes');
