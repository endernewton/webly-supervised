function options = canditWrapper(iid, options)
% wrapper for getting the candit images for testing (finding nearest neighbors)
...
for d=dindex
    ...
    for i=1:length(classes)
        ...
        tclasspath = [datasetcache,classname,'/',options.canditmethod,'/'];
        ...
        if ~fileExists(detfile) && makeDirOrFail(lockpath)
            ...
            for j=1:li
            	bar(j,li);
            	switch options.canditmethod
            	case 'full'
            		bboxes{j} = seedFull( imagelist{j}, j, options );
                case 'ebox'
                    bboxes{j} = seedEbox( imagelist{j}, j, options );
                case 'eboxt'
                    bboxes{j} = seedEboxThres( imagelist{j}, j, options );
                case 'rand'
                    bboxes{j} = seedRand( imagelist{j}, j, options);
            	otherwise
