...
void PReLULayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->blobs_.size() > 0) {
    ...
    if (channel_shared_) {
      this->blobs_[0].reset(new Blob<Dtype>(1, 1, 1, 1)); // all the filters share the same slope
    } else {
      this->blobs_[0].reset(new Blob<Dtype>(1, 1, 1, channels)); // different filters have different slope
    }
  ...
  multiplier_.Reshape(1, 1, 1, bottom[0]->count() / bottom[0]->num()); // channel x height x width
...
void PReLULayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  const Dtype* bottom_data = bottom_memory_.cpu_data();
  const Dtype* slope_data = this->blobs_[0]->cpu_data();
  const Dtype* top_diff = top[0]->cpu_diff();
  const int count = bottom_memory_.count();
  const int hw = bottom_memory_.height() * bottom_memory_.width();
  const int channels = bottom_memory_.channels();
